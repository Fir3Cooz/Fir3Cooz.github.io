...
function buildHeroText() {
  ...
  const labelText = "Développeur Web Junior";
  ...
  <div class="hero-line-row"><span class="word"><span class="word-inner">Lucas</span></span></div>
  <div class="hero-line-row"><span class="word"><span class="word-inner red">Hamon</span></span></div>
`;

  document.getElementById("heroSub").innerHTML = `
  <span class="word"><span class="word-inner">Développement Web</span></span>
  <span class="line-accent"></span>
  <span class="word"><span class="word-inner">Applications</span></span>
  <span class="line-accent"></span>
  <span class="word"><span class="word-inner">Fullstack</span></span>
`;
}
...
class SakuraEngine {
  ...
  update() {
    ...
    for (let i = this.petals.length - 1; i >= 0; i--) {
      ...
      const fadeOut =
        p.y > this.canvas.height - 100 ? (this.canvas.height - p.y) / 100 : 1;
      const alpha = p.opacity * fadeIn * Math.max(0, fadeOut);
      if (p.y > this.canvas.height + 20 || p.x < -50) {
...
class BookEngine {
  constructor() {
    ...
    if (piTotal)
      piTotal.textContent = String(this.numPages - 1).padStart(2, "0");
  }
...
function setupCursor() {
  ...
  animFollow();
  document
    .querySelectorAll("a, .proj-card, .skill-card, .magnetic-btn")
    .forEach((el) => {
